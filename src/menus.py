...
class GridMenu(GameMenu):
    def __init__(self, name, surface, width=env.MENU_WIDTH, height=env.MENU_HEIGHT, max_grid_width=50, max_grid_height=50):
    ...
    def controls(self):
        menu = pygame_menu.Menu(f'{self.name} Controls', env.MENU_WIDTH, env.MENU_HEIGHT, theme=pygame_menu.themes.THEME_SOLARIZED, onclose=pygame_menu.events.BACK)
...
class PathSmoothingMenu(GridMenu):
    def __init__(self, name, surface, width=env.MENU_WIDTH, height=env.MENU_HEIGHT, max_grid_width=20, max_grid_height=20):
